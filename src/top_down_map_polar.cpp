...
TopDownMapPolar::TopDownMapPolar(std::string path, cv::Mat& color_lut, int num_classes, int num_ex, float scale, float res)
  : TopDownMap(path, color_lut, num_classes, num_ex, scale, res)
{
  //Default
  samplePtsPolar(Eigen::Vector2i(100,50), 1, 2*M_PI/100);
}

void TopDownMapPolar::samplePtsPolar(Eigen::Vector2i shape, float res, float ang_res) {
...
void TopDownMapPolar::getLocalMap(Eigen::Vector2f center, 
  ...
  Eigen::Array2Xf pts = ang_sample_pts;
